...
class Conv2D(layers.Layer):
    ...
    * :math:`Out`: Output value, the shape of :math:`Out` and :math:`X` may be different.
    ...
    Parameters:
        ...
        use_mkldnn (bool, optional): Use mkldnn kernel or not, it is valid only when the mkldnn
            library is installed. Default: True.
    ...
    def __init__(self,
        ...
        if (self._num_channels == self._groups and
                num_filters % self._num_channels == 0
    ...
    def forward(self, input):
        if in_dygraph_mode() and self._l_type == 'conv2d':
            ...
            pre_act = dygraph_utils._append_bias_in_dygraph(pre_bias, self.bias, 1, 
                                                            use_mkldnn=self._use_mkldnn)
...
class Pool2D(layers.Layer):
    ...
    Parameters:
        ...
        use_mkldnn (bool, optional): Only used in mkldnn kernel, need install mkldnn. Default: True.
    ...
    def forward(self, input):
        if in_dygraph_mode():
            attrs = ('pooling_type', self._pool_type, 'ksize', self._pool_size,
                     ...
                     'use_mkldnn', self._use_mkldnn, 'exclusive', self._exclusive,
...
class Linear(layers.Layer):
    ...
    Parameters:
        ...
        use_mkldnn (bool, optional): Only used in mkldnn kernel, need to build with mkldnn. Default: False.
    ...
    def forward(self, input):
        if in_dygraph_mode():
            ...
            core.ops.matmul(input, self.weight, pre_bias, 'transpose_X', False,
                            'transpose_Y', False, "alpha", 1, "use_mkldnn", self._use_mkldnn)
            pre_act = dygraph_utils._append_bias_in_dygraph(
                pre_bias, self.bias, axis=len(input.shape) - 1, use_mkldnn=self._use_mkldnn)
        ...
        if self.bias is not None:
            ...
            self._helper.append_op(
                ...
                attrs={'axis': len(input.shape) - 1, "use_mkldnn": self._use_mkldnn})
        else:
            pre_activation = tmp
